...
Cold-start expectation:
    First request after deploy: ~30-60s (image pull + model load).
    Cold after scaledown: ~10-25s (model cached on Modal disk, just disk→VRAM).
    Warm: ~1-3s per request.
...
@app.function(
    ...
    # container down and the next request pays a ~15s cold start.
    ...
    # continuous batching.
    max_containers=1,
    # 5-min boot timeout matches what we used in modal_vllm_eval (vLLM
    # cold-load on A10G is normally 30-90s; 300s is generous headroom).
...
def vllm_server():
    ...
    cmd = [
        ...
        # --enforce-eager: same safe-mode flag as the eval script. Small
        # throughput cost vs CUDA graphs but eliminates a class of driver
        # mismatches and matches the canonical-eval engine config exactly.
        "--enforce-eager",
