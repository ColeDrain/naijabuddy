...
> This paper covers **Task A** of the DSN × BCT Hackathon 3.0 brief — User Modeling: given a persona, predict a star rating and generate an in-voice review for a target item. The same agent and codebase also serve **Task B** (Recommendation); that task is described in [`solution_paper_task_b.md`](solution_paper_task_b.md), and shares the architecture (§2), calibration layer (§2.3), and evaluation harness reported here. We submit one repo and two task-focused papers per the form's structure; the codebase is unified per the brief's "two tasks, one ambition" framing.
...
With more development time and computing resources, we propose the following scaling directions for Task A:
1. **History-conditioned calibration weights**: replace the per-domain constant weights with weights computed as an explicit function of `k` (number of observed user ratings), making the §4.3 regime switch automatic rather than a deployed best-of-sweep.
...
3. **Learned semantic critic**: replace the rule-based Critic Layer with a small classifier trained on (persona, item) → constraint-violation pairs, expanding coverage beyond keyword rules.

---

## 7. Conclusion (Task A)
In this paper, we presented **NaijaBuddy** for **Task A: User Modeling** — given a Nigerian persona and a target item, the agent produces a calibrated star rating and an in-voice review. By pairing an in-context `Qwen2.5-3B-Instruct` (served via interchangeable `llama-cpp-python` or vLLM backends — §2.2.1) with an adaptive three-term Calibration Layer ($\alpha\cdot\text{LLM} + \beta\cdot\mu_u + \gamma\cdot\mu_i$) and a deterministic Critic Layer, we deliver warm-user RMSE reductions of 6.3–13% over a global-mean baseline and **20–30% RMSE reductions for cold-start users at k = 1**. Review-text quality reaches BERTScore-F1 0.84 with a seed-variance floor of ±0.0003 — reproducible across three independent splits. A cross-domain transfer experiment (Amazon Books → Movies) shows the same item-bias signal rescues predictions even when the user is fully cold in the target domain. We evaluate with a leakage-free, reproducible harness and report results honestly, including the measured limit of the LLM's contribution to warm-user rating accuracy. Enriched with authentic Nigerian personas, NaijaBuddy's Task A path represents a reliable and culturally authentic approach to generative behavioral simulation on the edge.

The same agent serves **Task B: Recommendation**, described in [`solution_paper_task_b.md`](solution_paper_task_b.md), sharing the architecture, Critic Layer, and evaluation harness reported here.
