...
In this paper, we present **NaijaBuddy**, a 100% offline-first, containerized agentic recommendation and user modeling system tailored specifically for the Nigerian consumer market. Our architecture employs a dual-stage pipeline: (1) high-speed dense vector semantic search using `BAAI/bge-small-en-v1.5` over a hybrid SQLite catalog enriched with local establishments, movies, and literature; and (2) in-context reranking and review generation utilizing a quantized local GGUF engine (`Qwen-2.5-3B-Instruct`) hosted natively in-process via `llama-cpp-python`. To protect rating accuracy (RMSE), we implement a mathematical **Output Calibration Layer** that blends LLM estimates with user baseline means and uses a vector-based **Cluster-Mean** fallback for cold-start personas. Finally, a deterministic **Collaborative Critic Layer** applies rule-based enforcement of encoded behavioral constraints. Our leakage-free, out-of-sample evaluation over the full held-out set (n = 2,000 users per domain) establishes per-user calibration as a reliable RMSE anchor (up to an 11.9% reduction over a global-mean baseline) and shows the optimal LLM/statistics blend to be *adaptive*: the LLM's raw rating adds little for users with rich history, but cuts cold-start RMSE by 13–20%. We release the evaluation harness so that every reported figure reproduces.
...
**Robustness across seeds and sample size.** The warm-user result above — that the LLM's rating barely improves on the user-mean — has been stress-tested two independent ways. An earlier protocol evaluated three independent leave-one-out splits (seeds 42, 1, 7) at n = 350 and applied a *paired per-seed difference test* to the V1→V2 step: it was positive on **all three seeds** for Yelp (mean +0.018) and Goodreads (mean +0.005) — small but consistent — and indistinguishable from zero on Amazon (+0.002, one seed exactly 0.000). The n = 2,000 evaluation reported here — a single split with a six-fold-larger sample — reproduces the same picture: the V1→V2 step is +0.003 to +0.005 on every domain. However the data is resampled — three seeds, or one 6× split — the LLM's warm contribution stays within a few thousandths of an RMSE point. We offer that stability across a 6× scale change, rather than an error bar, as the evidence that the finding is real and not a sampling artefact.
...
| Domain | k = 1 | k = 2 | k = 3 | warm (§4.2) |
| :--- | :---: | :---: | :---: | :---: |
| Yelp | 1.405 → **1.131** (α 0.7) | 1.214 → 1.108 (α 0.5) | 1.135 → 1.065 (α 0.4) | 0.995 → 0.990 (α 0.1) |
| Goodreads | 1.245 → **1.030** (α 0.6) | 1.085 → 0.992 (α 0.5) | 1.042 → 0.977 (α 0.4) | 0.879 → 0.876 (α 0.1) |
| Amazon | 1.175 → **1.017** (α 0.6) | 1.036 → 0.969 (α 0.5) | 0.978 → 0.932 (α 0.4) | 0.856 → 0.851 (α 0.1) |

*Each cell: V1 user-mean RMSE → V2 best-blend RMSE, with the RMSE-optimal α. Cold-start is simulated by truncating each test user's history to k interactions; n = 2,000 users per domain per k (1,997 for Amazon), single leave-one-out split, seed 42. The warm column is §4.2.*

Two patterns hold across all three domains. First, **the RMSE-optimal $\alpha$ falls monotonically as history accumulates** — 0.6–0.7 at k = 1, 0.5 at k = 2, 0.4 at k = 3, and 0.1 once the user is warm — a strikingly consistent schedule across all three domains. Second, **the LLM blend's benefit grows sharply as history shrinks**: at k = 1 the optimal blend cuts RMSE by **19.5%** (Yelp, 1.405 → 1.131), **17.3%** (Goodreads, 1.245 → 1.030) and **13.4%** (Amazon, 1.175 → 1.017) over the user-mean — against the ≤ 0.6% warm gain of Section 4.2. By k = 3 the gain has already decayed to 4–6%; the LLM's value is concentrated almost entirely in the first one or two interactions.
...
NaijaBuddy's measured strengths are an **adaptive calibration layer** — a ~10% mean RMSE reduction over a global baseline for warm users and a 13–20% reduction for cold-start users — **hybrid retrieval** that beats the popularity baseline by 3.4–6× across the three domains, and fully offline operation. Its measured weaknesses are content-only retrieval, which does not beat popularity, and warm-user rating accuracy, where the LLM adds little — at n = 2,000 the user-mean + item-bias model is the warm result, and the LLM's numeric rating earns its place only in cold-start. We report all of these directly — including a small-sample figure from an earlier draft that the full evaluation overturned — because a recommender's credibility rests on an evaluation that reproduces. The warm figures are regenerated by `eval_harness.py --seeds 42 --bertscore`; the Section 4.5 cold-start curve by adding `--cold-start`; the §4.7 ablation with `--persona-mode rag --seed 42`.
...
**Cold-start.** Recommending for users or items with little history has a long line of classical remedies — content-based features, item-popularity and item-bias terms, cross-domain transfer, and meta-learning estimators such as MeLU [Lee et al., 2019], which meta-learns a preference model that adapts from only a handful of interactions. NaijaBuddy places the LLM *within* this lineage rather than against it: §4.5 shows the LLM acting as a cold-start specialist — its persona-grounded estimate substitutes for the statistics a new user has not yet generated — while the calibration layer hands control back to the classical user-bias and item-bias model as soon as history accumulates.
...
## 7. Conclusion
In this work, we developed **NaijaBuddy**, a highly localized, completely offline-first agentic recommender system built for the DSN x BCT Hackathon 3.0. By structuring our system around a dual-stage "Filter-then-Rerank" workflow, we achieve elite low-latency CPU inference within a self-contained Docker container. We implemented an adaptive Calibration Layer that anchors rating predictions to per-user statistics — reducing RMSE by up to 11.9% over a global-mean baseline, with the LLM blend cutting a further 13–20% for cold-start users — and a deterministic Critic Layer for rule-based constraint filtering. We evaluate the system with a leakage-free, reproducible harness over the full held-out set and report results honestly, including where it underperforms: content-only retrieval and warm-user rating accuracy. Enriched with authentic Nigerian personas, NaijaBuddy represents a reliable and culturally authentic approach to generative recommendation and consumer behavioral simulation on the edge.
