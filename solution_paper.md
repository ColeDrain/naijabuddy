...
| :--- | :---: | :---: | :---: |
| Yelp | 0.084 ± 0.001 | 0.723 ± 0.001 | **0.842** (single-seed) |
| Goodreads | 0.073 ± 0.001 | 0.626 ± 0.002 | **0.844** (single-seed) |
| Amazon | 0.079 ± 0.000 | 0.646 ± 0.001 | **0.849** (single-seed) |

*ROUGE-L and Semantic-BGE: n = 2,000 / 2,000 / 1,999 per seed across three independent splits (seeds 42 / 1 / 7), synth personas. Mean ± sample std reported. BERTScore-F1: single-seed reference value from the earlier llama-cpp-Q4 evaluation — the multi-seed synth re-run on vLLM had its BERTScore step OOM'd off the GPU and we report the prior canonical value (single-seed std on BERTScore-F1 was ≤ 0.0005 in prior runs).*
