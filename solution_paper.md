...
Two patterns hold across all three domains. First, **the RMSE-optimal $\alpha$ falls as history accumulates** — from 0.5–0.7 at k = 1 to 0.1 once the user is well-observed. Second, **the LLM blend's benefit grows as history shrinks**: at k = 1 the optimal blend cuts RMSE by 14.7% (Yelp, 1.308 → 1.116), 17.1% (Goodreads, 1.428 → 1.184) and 11.5% (Amazon, 1.077 → 0.953) over the user-mean — against the ≤ 0.6% warm gain of Section 4.2.
...
NaijaBuddy's measured strengths are an **adaptive calibration layer** — a ~10% mean RMSE reduction over a global baseline for warm users and a 11–17% reduction for cold-start users — **hybrid retrieval** that beats the popularity baseline by 3.4–6× across the three domains, and fully offline operation. Its measured weaknesses are content-only retrieval, which does not beat popularity, and warm-user rating accuracy, where the LLM adds little — at n = 2,000 the user-mean + item-bias model is the warm result, and the LLM's numeric rating earns its place only in cold-start. We report all of these directly — including a small-sample figure from an earlier draft that the full evaluation overturned — because a recommender's credibility rests on an evaluation that reproduces. The warm figures are regenerated by `eval_harness.py --seeds 42 --bertscore`; the Section 4.5 cold-start curve by adding `--cold-start`; the §4.7 ablation with `--persona-mode rag --seed 42`.
...
## 7. Conclusion
In this work, we developed **NaijaBuddy**, a highly localized, completely offline-first agentic recommender system built for the DSN x BCT Hackathon 3.0. By structuring our system around a dual-stage "Filter-then-Rerank" workflow, we achieve elite low-latency CPU inference within a self-contained Docker container. We implemented an adaptive Calibration Layer that anchors rating predictions to per-user statistics — reducing RMSE by up to 11.9% over a global-mean baseline, with the LLM blend cutting a further 11–17% for cold-start users — and a deterministic Critic Layer for rule-based constraint filtering. We evaluate the system with a leakage-free, reproducible harness over the full held-out set and report results honestly, including where it underperforms: content-only retrieval and warm-user rating accuracy. Enriched with authentic Nigerian personas, NaijaBuddy represents a reliable and culturally authentic approach to generative recommendation and consumer behavioral simulation on the edge.
